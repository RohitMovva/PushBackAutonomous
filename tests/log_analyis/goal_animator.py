...
with open('robot_log_19700101_000000.txt', 'r') as file:
    for line in file:
        ...
        if time_match:
            ...
            elif 'MP Output:' in line:
                # Extract velocity data
                parts = line.split('MP Output:')[1].strip().split()
                if len(parts) >= 2:
                    linear_vel.append(float(parts[0]))
                    angular_vel.append(float(parts[1]))
...
# Ensure we have equal number of goal poses and velocities
min_length = min(len(goal_x), len(linear_vel))
goal_x = goal_x[:min_length]
goal_y = goal_y[:min_length]
goal_theta = goal_theta[:min_length]
linear_vel = linear_vel[:min_length]
angular_vel = angular_vel[:min_length]
...
sim_line, = ax.plot([], [], 'g-', label='Simulated Path', linewidth=2)
...
def init():
    ...
    ax.set_title('Robot Goal Path vs. Velocity-Integrated Path')
    ax.legend()
    goal_line.set_data([], [])
    goal_point.set_data([], [])
    sim_line.set_data([], [])
    ...
    time_text.set_text('')
    error_text.set_text('')
    return goal_line, goal_point, sim_line, sim_point, time_text, error_text
...
def animate(i):
    ...
    error_text.set_text(f'Position Error: {pos_error:.2f}\", Angle Error: {angle_error:.2f} rad')
    
    return goal_line, goal_point, sim_line, sim_point, time_text, error_text, goal_arrow, sim_arrow
